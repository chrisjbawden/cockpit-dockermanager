...
# Quick installer for SUSE/openSUSE systems

set -e

echo "🐳 Installing cockpit-dockermanager for Green distros..."
...
if ! command -v zstd &> /dev/null; then
    ...
    sudo zypper install -y zstd >/dev/null 2>&1 || {
...
if ! command -v ar &> /dev/null; then
    ...
    sudo zypper install -y binutils >/dev/null 2>&1 || {
